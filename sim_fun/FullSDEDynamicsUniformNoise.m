...
if ~isfield(DynParams,'TMSTime')
    DynParams.TMSTime = 1e6;
end
...
thetam = 0:2*pi/MemoryNet.N:2*pi-pi/MemoryNet.N;
...
for ii = 1:round(DynParams.PFTime/dt)
    ...
    SensoryInputI = SensoryNet.Conn*SS_old + MemoryNet.MBackward*SM_old + I0'*(ii<(DynParams.PFTime/dt));
...
if strcmp(DynParams.DecodingFrom,'Memory')
    ...
    PF = ComputePF(mSensory_new,SampleInput);
end
    ...
    for ll = 1:DynParams.RepTime
        ...
        for ii = 1:step
            ...
            SensoryInputI = SensoryNet.Conn*SS_old + MemoryNet.MBackward*SM_old ...
                + I0'*(ii<(StimTime/dt));
            mSensory_new = SensoryNet.q(SensoryInputI);
            MemoryInput = MemoryNet.Conn*MM_old + MemoryNet.IEc + SensoryNet.MForward*MS_old*TMSFlag;
            mMemory_new = MemoryNet.q(MemoryInput);

            if ii == round(DecodeTime(DecodeCounter)/dt)
                for jj = 1:NInputSample
                     if strcmp(DynParams.Decoder,'Unaware')
                        if strcmp(DynParams.DecodingFrom,'Memory')
                            DecodedOrientation(ll,jj,DecodeCounter) = PVDecoder(thetam,mMemory_new(:,jj));
                        else
                            DecodedOrientation(ll,jj,DecodeCounter) = PVDecoder(thetas,mSensory_new(:,jj));
                        end
                    else
                        if strcmp(DynParams.DecodingFrom,'Memory')
                            DecodedOrientation(ll,jj,DecodeCounter) = PVDecoder(PF,mMemory_new(:,jj));
                        else
                            DecodedOrientation(ll,jj,DecodeCounter) = PVDecoder(PF,mSensory_new(:,jj));
                        end
                    end
                end
        ...
        for ii = 1:step
            ...
            if ii == round(DecodeTime(DecodeCounter)/dt)
                for jj = 1:NInputSample
                    if strcmp(DynParams.Decoder,'Unaware')
                        if strcmp(DynParams.DecodingFrom,'Memory')
                            DecodedOrientationTemp(jj,DecodeCounter) = PVDecoder(thetam,mMemory_new(:,jj));
                        else
                            DecodedOrientationTemp(jj,DecodeCounter) = PVDecoder(thetas,mSensory_new(:,jj));
                        end
                    else
                        if strcmp(DynParams.DecodingFrom,'Memory')
                            DecodedOrientationTemp(jj,DecodeCounter) = PVDecoder(PF,mMemory_new(:,jj));
                        else
                            DecodedOrientationTemp(jj,DecodeCounter) = PVDecoder(PF,mSensory_new(:,jj));
                        end
                    end
                end
                if DecodeCounter < length(DecodeTime)
