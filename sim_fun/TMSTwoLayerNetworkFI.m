function [FI,SensoryNet,MemoryNet,DynParams] = TMSTwoLayerNetworkFI(SensoryNet,MemoryNet,DynParams)
...
TuningCurves4 = zeros(MemoryNet.N,NInputSample,length(DecodeTime));
...
if DynParams.Parallel == 0
    for ll = 1:DynParams.RepTime
        ...
        for ii = 1:step
            ...
            MemoryInput = MemoryNet.Conn*MM_old + MemoryNet.IEc + SensoryNet.MForward*MS_old*(ii<(Dynparams.TMSTime/dt));
    ...
    parfor ll = 1:DynParams.RepTime
        ...
        for ii = 1:step
            ...
            SensoryInputI = SensoryNet.Conn*SS_old + MemoryNet.MBackward*SM_old ...
                + I0'*(ii<(StimTime/dt));
            mSensory_new = SensoryNet.q(SensoryInputI);
            MemoryInput = MemoryNet.Conn*MM_old + MemoryNet.IEc + SensoryNet.MForward*MS_old*(DynParams.TMSTime<(ii/dt));
            mMemory_new = MemoryNet.q(MemoryInput);
...
for uu = 1:4
    switch uu
        ...
        case 4
            TCMean = mean(TuningCurves4,3);
            TCVar = var(TuningCurves4,0,3);
    end
