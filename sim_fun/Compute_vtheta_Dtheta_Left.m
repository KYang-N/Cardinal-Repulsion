function [vtheta,Dtheta,ThetaLoc] = Compute_vtheta_Dtheta_Left(SensoryNet,MemoryNet,DynParams)
...
u_left = zeros(4*MemoryNet.N,NInputSample);
...
for i = 1:NInputSample
    v(i) = 1/(Tau_syn)*u_left(:,i)'*(-s_manifold(:,i)+phiOut(:,i)); % s
    uSquare = u_left(:,i).^2;
