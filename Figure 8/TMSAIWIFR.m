%% TMS asymmetry index and width index -- Figure 8
...
%% Without TMS
    ...
    TMSTime = 1e4; % Set a large time so the TMS can't be reached
    tic
    mSensory_old = zeros(Ns,NInputSample);
    mMemory_old = zeros(Nm,NInputSample);
    SS_old = zeros(Ns,NInputSample);
...
%% Data analysis
    ...
    FRMaxSensoryWithoutTMS = zeros(1,length(PlotTime));
    for i = 1:length(PlotTime)
        FRMaxSensoryWithoutTMS(i) = max(RateSensory(PlotNeuronInd,:,i));
    end

    FRMaxMemoryWithoutTMS = zeros(1,length(PlotTime));
    for i = 1:length(PlotTime)
        FRMaxMemoryWithoutTMS(i) = max(RateMemory(PlotNeuronInd,:,i));
    end
% Compute asymmetry index

    AsyIndMemoryWithoutTMS = zeros(1,length(PlotTime));
    for i = 1:length(PlotTime)
        AsyIndMemoryWithoutTMS(i) = AsymmetryIndex(SampleInput,RateMemory(PlotNeuronInd,:,i));
    end

    AsyIndSensoryWithoutTMS = zeros(1,length(PlotTime));
    for i = 1:length(PlotTime)
        AsyIndSensoryWithoutTMS(i) = AsymmetryIndex(SampleInput,RateSensory(PlotNeuronInd,:,i));
    end
% Compute sensory biases

    BiasSensoryWithoutTMS = zeros(1,length(PlotTime));
    for Time = 1:length(PlotTime)
        for kk = 1:NInputSample
            BiasSensoryWithoutTMS(kk,Time) = PVDecoder(PFSensory,RateSensory(:,kk,Time))-SampleInput(kk);
        end
    end
    BiasSensoryWithoutTMS(BiasSensoryWithoutTMS>pi) = BiasSensoryWithoutTMS(BiasSensoryWithoutTMS>pi) - 2*pi;
    BiasSensoryWithoutTMS(BiasSensoryWithoutTMS<-pi) = BiasSensoryWithoutTMS(BiasSensoryWithoutTMS<-pi) + 2*pi;

    BiasSensoryWithoutTMS = 0.5*BiasSensoryWithoutTMS/pi*180;
    SampleBiasSensoryWithoutTMS = BiasSensoryWithoutTMS(round(NInputSample/8),:);
% Compute memory biases

    BiasMemoryWithoutTMS = zeros(1,length(PlotTime));
    for Time = 1:length(PlotTime)
        for k = 1:NInputSample
            BiasMemoryWithoutTMS(k,Time) = PVDecoder(PFMemory,RateMemory(:,k,Time))-SampleInput(k);
        end
    end
    BiasMemoryWithoutTMS(BiasMemoryWithoutTMS>pi) = BiasMemoryWithoutTMS(BiasMemoryWithoutTMS>pi) - 2*pi;
    BiasMemoryWithoutTMS(BiasMemoryWithoutTMS<-pi) = BiasMemoryWithoutTMS(BiasMemoryWithoutTMS<-pi) + 2*pi;

    BiasMemoryWithoutTMS = 0.5*BiasMemoryWithoutTMS/pi*180;

    SampleBiasMemoryWithoutTMS = BiasMemoryWithoutTMS(round(NInputSample/8),:);
% Compute tuning width index

    TuningWidthSensoryWithoutTMS = zeros(length(PlotTime),Ns);
    WidthContrastSensoryWithoutTMS = zeros(1,length(PlotTime));
    for i = 1:length(PlotTime)
        for j = 1:Ns
            TuningWidthSensoryWithoutTMS(i,j) = ComputeTuningWidth(SampleInput,RateSensory(j,:,i));
        end
        MaxWidth = max(TuningWidthSensoryWithoutTMS(i,:));
        MinWidth = min(TuningWidthSensoryWithoutTMS(i,:));
        WidthContrastSensoryWithoutTMS(i) = (MaxWidth-MinWidth)/(MaxWidth+MinWidth);
    end

    TuningWidthMemoryWithoutTMS = zeros(length(PlotTime),Nm);
    WidthContrastMemoryWithoutTMS = zeros(1,length(PlotTime));
    for i = 1:length(PlotTime)
        for j = 1:Nm
            TuningWidthMemoryWithoutTMS(i,j) = ComputeTuningWidth(SampleInput,RateMemory(j,:,i));
        end
        MaxWidth = max(TuningWidthMemoryWithoutTMS(i,:));
        MinWidth = min(TuningWidthMemoryWithoutTMS(i,:));
        WidthContrastMemoryWithoutTMS(i) = (MaxWidth-MinWidth)/(MaxWidth+MinWidth);
    end
%% Save data
    ...
    if SaveFlag
       ...
       DataDir = '';
        save([DataDir,'TMSAsymmetryTuningWidthFR','Mode',Mode,'alpha',strrep(num2str(alpha),'.','p'),'beta',strrep(num2str(beta),'.','p')]);
    end
end
% Plot bias increase
...
if LoadFlag
    ...
    clear
     DataDir = '';
    load([DataDir,'TMSAsymmetryTuningWidthFRModeBothalpha0p03beta0p08.mat']);
end
TMSTime = 2.5;
MemColor = '#5C0B72';
SenColor = '#448983';
...
plot(TimeInSecond,SampleBiasSensoryWithoutTMS,'--','LineWidth',2,'Color',SenColor);
hold on
plot(TimeInSecond,SampleBiasSensory,'LineWidth',2,'Color',SenColor);
plot(TimeInSecond,SampleBiasMemoryWithoutTMS,'--','LineWidth',2,'Color',MemColor);
plot(TimeInSecond,SampleBiasMemory,'LineWidth',2,'Color',MemColor);
hold off
xline(StimTime,'Color',[0.3,0.3,0.3],'LineWidth',1.2);
ylim([0,max(SampleBiasMemoryWithoutTMS)+0.05]);
...
figure(f3)
TimeInSecond = PlotTime;
plot(TimeInSecond,FRMaxSensoryWithoutTMS,'--','LineWidth',2,'Color',SenColor);
hold on
plot(TimeInSecond,FRMaxSensory,'LineWidth',2,'Color',SenColor);
plot(TimeInSecond,FRMaxMemoryWithoutTMS,'--','LineWidth',2,'Color',MemColor);
...
f6 = figure;
figure(f6)
plot(TimeInSecond,WidthContrastSensoryWithoutTMS,'--','LineWidth',2,'Color',SenColor);
hold on;
plot(TimeInSecond,WidthContrastSensory,'LineWidth',2,'Color',SenColor);
plot(TimeInSecond,WidthContrastMemoryWithoutTMS,'--','LineWidth',2,'Color',MemColor);
