%% Search alpha and sigma to investigate the bias and SD profile for the two-layer model
...
%% Define connectivity and background input
    ...
    step = round(tmax/dt);
    SDIndex = zeros(length(alphaRange),length(sigmaRange));
    MaxBias = SDIndex;
    ...
    for j = 1:length(alphaRange)
        for i = 1:length(sigmaRange)
            ...
            parfor jj = 1:RepTime
                ...
                for ii = 1:step
                    SS_new = SS_old+ 1/Tau_syn*dt*(-SS_old+mSensory_old)+...
                        1/Tau_syn*sqrt(dt)*sigma*randn(Ns,NumInput); 
                    SM_new = SM_old + 1/Tau_syn*dt*(-SM_old+mMemory_old)+...
                        1/Tau_syn*sqrt(dt)*sigma*randn(Nm,NumInput);
                    MS_new = MS_old + 1/Tau_syn*dt*(-MS_old+mSensory_old)+...
                        1/Tau_syn*sqrt(dt)*sigma*randn(Ns,NumInput);
                    MM_new = MM_old + 1/Tau_syn*dt*(-MM_old+mMemory_old)+...
                        1/Tau_syn*sqrt(dt)*sigma*randn(Nm,NumInput);
    ...
    delete(p)
    if SaveFlag 
        save('TwoLayerBiasSDSearchalphasigma.mat');
    end
end
%% Visualize results
...
if LoadFlag
    clear %#ok<*UNRCH> 
    load TwoLayerBiasSDSearchalphasigma.mat;
end
...
nexttile(1)
imagesc(sigmaRange,alphaRange,MaxBias);
h = colorbar;
set(h,'LineWidth',0.8);
h.TickDirection = 'out';
...
box off
axis square
set(gcf,'Unit','Centimeters','Position',[2,2,7,7]);


nexttile(2)
imagesc(sigmaRange,alphaRange,SDIndex);
h = colorbar;
h.TickDirection = 'out';
h.Ticks = [-0.1,0,0.25];
set(h,'LineWidth',0.8);
...
yticks(0.02:0.03:0.08);
xticks([1e-3,1e-2,1e-1,1]);
set(gcf,'Unit','Centimeters','Position',[2,2,14,7]);
box off
axis square