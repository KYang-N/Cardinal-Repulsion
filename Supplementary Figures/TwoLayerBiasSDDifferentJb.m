...
%% Dynamics of the full SDE
    ...
    DecodedValue = zeros(DynParams.RepTime,DynParams.NInputSample,length(JBackwardRange));
    ...
    if SaveFlag
        ...
        save(['PoissonNoiseResultsDifferentJb','Mode',SensoryNet.Mode,'.mat']);
    end
end
%%
if LoadFlag
    clear %#ok<*UNRCH> 
    DataDir = '';
    load([DataDir,'PoissonNoiseResultsDifferentJbModeEOnly.mat']);
end
close
...
BiasMean = zeros(length(JBackwardRange),NInputSample);
StdDv = BiasMean;
SEM = StdDv;
