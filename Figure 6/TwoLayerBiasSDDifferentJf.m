...
%% Dynamics of the full SDE
    ...
    JForwardRange = [0.07,0.10,0.13];
    DecodedValue = zeros(DynParams.RepTime,DynParams.NInputSample,length(JForwardRange));
    ...
    if SaveFlag
        ...
        save(['PoissonNoiseResultsDifferentJf','Mode',SensoryNet.Mode,'.mat']);
    end
end
%%
if LoadFlag
    clear %#ok<*UNRCH> 
    DataDir = '';
    load([DataDir,'PoissonNoiseResultsDifferentJfModeEOnly.mat']);
end
close
...
BiasMean = zeros(length(JForwardRange),NInputSample);
StdDv = BiasMean;
SEM = StdDv;

for Time = 1:length(JForwardRange)
...
%% Std
Std = zeros(length(JForwardRange),NInputSample);
for Time = 1:length(JForwardRange)
...
xlabel('\theta (deg)');
ylabel('SD (deg)');
set(f1,'Units','Centimeters','Position',[2,2,9,4]);
